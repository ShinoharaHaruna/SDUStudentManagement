...
public class TeachController {
    //Java 对象的注入 我们定义的这些Java的操作对象都不能自己管理，是由有Spring框架来管理的， TeachController 中要使用StudentRepository接口的实现类对象，
    ...
    //
    // Map 对象是存储数据的集合类，框架会自动将Map转换成用于前后台传输数据的Json对象，Map的嵌套结构和Json的嵌套结构类似，
...
//  学生信息提交按钮方法
    ...
    //根据学号查询创新记录
    public List getInnovationMapList (String innoNum){
        List dataList = new ArrayList();
        List<Innovation> sList = innovationRepository.findInnovationListByNumName(innoNum);
        if(sList == null || sList.size() == 0)
            return dataList ;
        Innovation s ;
        Map m ;
        for(int i = 0 ; i < sList.size() ; i++){
            ...
            switch(s.getInnoType()){
                case "1" : m.put("innoType","社会实践");break;
                case "2" : m.put("innoType","学科竞赛");break;
                case "3" : m.put("innoType","科技成果");break;
                case "4" : m.put("innoType","培训讲座");break;
                case "5" : m.put("innoType","创新项目");break;
                case "6" : m.put("innoType","校外实习");break;
            }
            m.put("innoName",s.getInnoName()) ;
            m.put("birthday" , DateTimeTool.parseDateTime(s.getInnoDate(),"yyyy-MM-dd"));
            dataList.add(m);
        }
        return dataList ;
    }
    ...
    public DataResponse innovationEditSubmit(@Valid @RequestBody DataRequest dataRequest){
        ...
        String innoType = CommonMethod.getString(form,"innoType");
        Date innoDate = CommonMethod.getDate(form,"innoDate");

        //还需要对学生进行检查，若学生库里无此学生，则要报错，令学生去学生管理界面录入自己的信息
        Innovation i  = null ;
        Optional<Innovation> op ;

        List<Student> sList = studentRepository.findStudentListByNumName(studentNum);
        if(sList == null || sList.size() == 0 ){
            return CommonMethod.getReturnMessageError("不存在该同学");
        }
        ...
        i.setInnoDate(innoDate);
        i.setInnoName(innoName);
        i.setInnoType(innoType);
        i.setStudentName(studentName);
        i.setStudentNum(studentNum);

        innovationRepository.save(i);
        return CommonMethod.getReturnData(i.getId());
    }
